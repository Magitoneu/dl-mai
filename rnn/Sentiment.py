...
if __name__ == '__main__':
    ...
    Tweet = pandas.read_csv("../datasets/amazon_alexa.tsv", delimiter='\t')
#    Tweet = pandas.read_csv("Presidential.csv")

    # Pre-process the tweet and store in a separate column
    Tweet['clean_tweet'] = Tweet['verified_reviews'].apply(lambda x: tweet_to_words(x))
    # Convert sentiment to binary
    Tweet['sentiment'] = Tweet['rating'].apply(lambda x: x - 1)
    ...
    numwords = 500  # Limit the number of words to use
